...
const FeaturedRecipes = ({ showAll }) => {
  ...
  useEffect(() => {
    if (featuredRecipeToNavigate) {
      navigation.navigate(
        featuredRecipeToNavigate?.name[selectedLanguage]
      );
  ...
  const renderPlaceholders = () => {
    ...
    return placeholders.map((_, index) => (
      ...
      >
        <Animated.View
          style={[styles.placeholderImage, { opacity: shimmerValue }]}
        />
        <View style={styles.placeholderOverlay}>
          <Animated.View
            style={[styles.placeholderText, { opacity: shimmerValue }]}
          />
        </View>
      </View>
    ));
  ...
  const calculateCardSize = (showAll) => {
    let cardWidth, cardHeight;
    if (showAll) {
      const numColumns = 2;
      const totalHorizontalPadding = 32;
      const totalMarginBetweenCards = 20;
      const availableWidth = screenWidth - totalHorizontalPadding - totalMarginBetweenCards;
      cardWidth = availableWidth / numColumns;
      cardHeight = screenHeight < 1000 ? 170 : 240;
    } else {
      cardWidth = screenWidth < 600 ? screenWidth / 1.6 : screenWidth / 2.15;
      cardHeight = screenHeight < 1000 ? 170 : 240;
    }
  ...
  const { cardWidth, cardHeight } = calculateCardSize(showAll);
      ...
      <SafeAreaView style={[styles.safeArea, dynamicSafeAreaStyle]}>
        <View style={styles.headerNavButton}>
          ...
          <Text
            style={[
              styles.text,
              dynamicPageTitleStyle,
              { fontSize },
            ]}
          >
            {languageStore[selectedLanguage]['featured_recipes']?.toUpperCase() ||
              ''}
          </Text>
        </View>
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => handleOnPressFeaturedRecipe(item)}
              >
                <Image
                  source={
                    item?.imageUrl !== ''
                ...
                />
                <Text style={[styles.recipeName, { fontSize }]}>
  ...
  return (
    <View>
      <View style={styles.header}>
        ...
        >
          {languageStore[selectedLanguage]['featured_recipes']}
        </Text>
        {!loading && (
          <TouchableOpacity
            onPress={() =>
              navigation.navigate(
                languageStore[selectedLanguage]['all_featured_recipes']
              )
            }
          >
            <Text
            ...
            >
              {languageStore[selectedLanguage]['see_all']}
            </Text>
          </TouchableOpacity>
        )}
      </View>
        ...
        {loading
          ...
          : featuredRecipes.slice(0, 5)?.map((recipe, index) => (
              <TouchableOpacity
                key={index}
                style={[
                  styles.featuredCardHome,
                  { width: cardWidth, height: cardHeight },
                ]}
                onPress={() => handleOnPressFeaturedRecipe(recipe)}
              >
                <Image
                  source={
                    recipe?.imageUrl !== ''
